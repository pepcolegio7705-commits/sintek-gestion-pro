<?php
   ...
   if($_SESSION['rol'] != 1 && $_SESSION['rol'] != 2 ){
    ...
    if(!empty($_POST)){
        ...
        if(openssl_decrypt($_POST['idusuario'],AES,KEY) == 1){
            header("Location:lista_usuarios.php");
            mysqli_close($con);
            exit;
        }
        $idusuario = openssl_decrypt($_POST['idusuario'],AES,KEY);
        //$query = mysqli_query($con, "DELETE from usuario where idusuario=$idusuario");
        $query_delete = mysqli_query($con, "UPDATE usuarios set estatus = 0 where idusuario = $idusuario");
        mysqli_close($con);
        if($query_delete){
                  $mensaje = '<script>
                  Swal.fire({
            ...
            $mensaje =  '<script>
                  ...
                  icon: "Error",
                  title: "Oops...",
                  text: "Error al eliminar el Usuario - Comuniquese con el administrador!"
                        });
               </script>';
        }
    }
...
<?php
    ...
    if(empty(openssl_decrypt($_GET['id'],AES,KEY)) || openssl_decrypt($_GET['id'],AES,KEY) == 1){ //Validamos que el id no este vacio y que en la barra de navegación si pongo id=1 me devuelva a la lista para no el administrador
        header("Location:lista_usuarios.php");
        mysqli_close($con);
    }
    ...
    $idusuario = openssl_decrypt($_GET['id'],AES,KEY);

    $query_consulta = mysqli_query($con, "SELECT idusuario from usuarios where idusuario = '$idusuario' and estatus=1");
    $resultado = mysqli_num_rows($query_consulta);
    ...
    $query = mysqli_query($con, "SELECT u.nombre, u.usuario, r.rol from usuarios u inner JOIN rol r on u.rol = r.idrol where u.idusuario = $idusuario order by idusuario DESC");
    mysqli_close($con);
                            ...
                            <div class="block-content text-center">
                              ...
                              <div class="">
                                   ...
                                   <form method="post" action="">
                                     <input type="hidden" name="idusuario" value="<?php echo openssl_encrypt($idusuario,AES,KEY);?>">
                                     <a href="lista_usuarios.php" class="btn btn-success"><i class="fas fa-ban"></i> Cancelar</a>
                                     <button type="submit" class="btn btn-danger"><i class="fas fa-user-minus"></i> Eliminar</button>
                                   </form>
                                 </div>
