...
if ($id_staff === 0) die("ID de personal no valido");

// 1. Datos completos del Staff y su Área
$stmt_staff = $pdo->prepare("SELECT s.*, a.nombre_area 
                             FROM personal_staff s 
                             LEFT JOIN areas a ON s.id_area = a.id_area 
                             WHERE s.id_staff = :id");
$stmt_staff->execute([':id' => $id_staff]);
$s = $stmt_staff->fetch(PDO::FETCH_ASSOC);
if (!$s) die("Personal no encontrado");

// 2. Retenciones Judiciales (Específicas para Staff)
...
// --- BLOQUE 4: RETENCIONES JUDICIALES ---
...
// --- BLOQUE 5: FAMILIA Y DOCUMENTACIÓN ---
